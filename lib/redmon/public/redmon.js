var Redmon = (function() {
  ...
  function requestData(count, callback) {
    $.ajax({
      url: config.absoluteUrl + 'stats?redis_instance=' + config.redisInstance + '&count=' + count,
  ...
  var toolbar = (function() {
    ...
    function onBtnClick(cmd) {
      $.ajax({url: config.absoluteUrl + 'cli?redis_instance=' + config.redisInstance + '&command=' + cmd});
    }
  ...
  var configWidget = (function() {
    ...
    function render() {
      $('#config-table .editable').each(function() {
        ...
        var options = {
          url           : config.absoluteUrl + 'config?redis_instance=' + config.redisInstance,
          element_id    : 'param',
          update_value  : 'value',
          show_buttons  :  true,
  ...
  var cli = (function() {
    ...
    function process(command, callback) {
      ...
      $.ajax({
        url     :  config.absoluteUrl + 'cli?redis_instance=' + config.redisInstance + '&command=' + command,
        success :  callback
      });
    }
